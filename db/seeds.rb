...
string_methods = %w[byteindex byteslice capitalize casecmp casecmp? center chars chomp chop chr clear concat count delete delete_prefix delete_suffix downcase dump each_char each_line empty? end_with? eql? gsub hash hex include? index insert inspect length lines ljust lstrip match match? next oct ord partition prepend replace reverse rindex rjust rpartition rstrip scan scrub size slice split squeeze start_with? strip sub sum swapcase tr tr_s undump upcase upto]
